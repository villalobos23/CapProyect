...
```{r cache=TRUE}
sampleSize <- 0.1
en_US.blog.sample <- iconv(readNLines(en_US.blog.file,sampleSize*en_US.blog.lines),'UTF-8', 'ASCII')
en_US.twitter.sample <-  iconv(readNLines(en_US.twitter.file,sampleSize*en_US.twitter.lines),'UTF-8', 'ASCII')
en_US.news.sample <- iconv(readNLines(en_US.news.file,sampleSize*en_US.news.lines),'UTF-8', 'ASCII')
en_US.blog.doc <- PlainTextDocument(en_US.blog.sample)
en_US.twitter.doc <- PlainTextDocument(en_US.twitter.sample)
en_US.news.doc <- PlainTextDocument(en_US.news.sample)
...
```{r}
head(en_US.blog.sample)
```
Head of the twitter Sample
```{r}
head(en_US.twitter.sample)
```
Head of the news Sample
```{r}
head(en_US.news.sample)
```
##Cleaning Data
